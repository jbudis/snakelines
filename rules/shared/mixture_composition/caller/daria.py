...
for f in sys.argv[1:]:
    ...
    with open(f, 'r') as infile:
        ...
        result.loc[len(result.index)] = [f[:-4], 'delta weighted average: ' + avg[0], 'omicron (21K) weighted average: ' + avg[1], 'omicron (21L) weighted average: ' + avg[2], '']
        result.to_csv(f[:-4] + '.tsv', sep='\t', index=False)
        summary.append([f[:-4]] + avg)
...
df_summary.to_csv('summary.tsv', sep='\t', index=False)
