...
def create_working_accessories(input_file):
    """
    Creates working directory and file from input absolute path to file
    :param input_file:
    :return: None
    """
    working_dir = list()
...
def create_tpm_table(input_file, dictionary, abundance_file, krona_table):
    """
    Creates final tpm table for krona
    ...
    # Load abundance file and search into this file ncbi_id from input file, then save ncbi_id to dictionary
    ...
    load_krona_table = load_krona_table.drop(0, 1)  # Remove column 0 on axis 1 = it means, it can delete only columns
    
    load_krona_table.rename(columns={1: load_krona_table[1][1],
                                     2: load_krona_table[2][1],
                                     3: load_krona_table[3][1],
                                     4: load_krona_table[4][1]}, inplace=True)

    load_krona_table = load_krona_table.drop([0, 1])    

    if load_krona_table.isnull().values.any():
